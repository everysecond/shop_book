        ...
        <div class="layui-card" style="padding: 25px;height:80%;">
            <div style="margin-bottom: 10px">
                <button class="cps-btn mybtn btn-batch" for="LAY-user-back-manage"
                        data-confirm="是否确认取消选中的计划？"
                        href="{{route('plan.destroy','batch')}}" data-method="delete">取消计划
                </button>
            </div>
            ...
            table.render({
                ...
                cols: [
                    [
                        ...
                        {field: "follow_at", title: "跟进时间", width: '15%'},
                        {
                            title: "操作",
                            align: "center",
                            fixed: "right",
                            templet: function (item) {
                                if (type == "myself") {
                                    var edit = common.route('{{route('plan.edit',':id')}}', {id: item.id});
                                    return '<a class="layui-btn layui-btn-normal layui-btn-xs open-frame" ' +
                                        ...
                                        '<i class="layui-icon layui-icon-edit"></i>转为记录</a>';
                                } else {
                                    return "";
                                }
                            }
                ...
                where: {orderBy: 'follow_at', sortedBy: 'desc', record_type: 2}
            });
