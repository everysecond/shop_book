...
class StartLogController extends Controller
{
    ...
    {
        $timeType = dateType();
        return view("manage::lease.report.user.start_chart", compact("timeType"));
    }
    ...
    public function startFrom(Request $request)
    {
        try {
            ...
            $modelData = LeaseStartTerminalLog::selectRaw("sum(user_ios_num) as user_ios_num,sum(user_android_num) as user_android_num");
            $modelData = $modelData->whereBetween("date", $defaultDay)->first();
            $modelDataFormat = [];
            $modelArr = [];
            if ($modelData) {
                if ($modelData->user_ios_num) {
                    $modelDataFormat[] = [
                        "value" => $modelData->user_ios_num,
                        "name"  => "用户端IOS"
                    ];
                    $modelArr[] = "用户端IOS";
                }
                if ($modelData->user_android_num) {
                    $modelDataFormat[] = [
                        "value" => $modelData->user_android_num,
                        "name"  => "用户端安卓"
                    ];
                    $modelArr[] = "用户端安卓";
                }
    ...
    {
        try {
            $data = LeaseStartLog::getSumData($request);
            return result("", 0, $data["data"], $data["count"]);
        } catch (\Exception $exception) {
            return result($exception->getMessage(), -1);
        }
