...
class LeaseCusEventLogRepository extends BaseRepository
{
    ...
    public function activeTrend($request, &$defaultDay)
    {
        ...
        $data = $this->makeModel()->newQuery()->selectRaw("sum(times) as num,date,url_name as page_url");
        if ($agentId = $request->agentId) {
            $data->where("province_id", $agentId);
        }
        return $data->whereBetween("date", $defaultDay)->groupBy("date")
            ->groupBy("url_name")->orderBy("date")->get()->toArray();
    }
    ...
    public function activeData($request, $defaultDay)
    {
        ...
        $data = $this->makeModel()->newQuery()->selectRaw("sum(times) as num,url_name as page_url");
        if ($agentId = $request->agentId) {
            $data->where("province_id", $agentId);
        }
        return $data->whereBetween("date", $defaultDay)->groupBy("url_name")->get()->toArray();
    }
    ...
    public function activeList($request)
    {
        ...
        $data = $this->makeModel()->newQuery()
            ->selectRaw("url_name as page_url,date,sum(user_num) as user_num,sum(times) as times")
            ->whereIn("date", $dateArr);
        if ($agentId = $request->agentId) {
            $data->where("province_id", $agentId);
        }
        $data = $data->groupBy("date")->groupBy("url_name")
            ->orderBy("date", "desc")->get()->toArray();
        $return = [];
        $urlArr = config("global.user_active_event_url");
