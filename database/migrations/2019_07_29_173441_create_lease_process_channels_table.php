...
class CreateLeaseProcessChannelsTable extends Migration {
	...
	public function up()
	{
		Schema::create('lease_process_channels', function(Blueprint $table)
		{
			$table->integer('id', true);
			$table->date('process_date')->index('renewal_date')->comment('日期（例如 2019-7-10）');
			$table->char('systemtype', 10)->comment('所属于渠道(Android or ios)');
			...
			$table->integer('mylease_num')->default(0)->comment('我的租赁详情');
        ...
        \Illuminate\Support\Facades\DB::statement("ALTER TABLE `lease_process_channels` comment '租赁渠道转化记录表'");//表注释
	}
	...
	{
		Schema::drop('lease_process_channels');
	}
