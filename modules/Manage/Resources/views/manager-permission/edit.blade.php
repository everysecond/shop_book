...
@section('content')
    <div class="layui-form" lay-filter="form" id="form" style="padding: 20px 30px 0 0;">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                @if($parent)
                    {{Form::select('pid',[$parent->id => $parent->name],'',['disabled'])}}
                @else
                    {{Form::select('pid',[0=>'根模块'],'',['disabled'])}}
                @endif
            </div>
        </div>
    ...
    <script>
        ...
        layui.extend({
            ...
            form.on("submit(form-submit)", function (data) {
                ...
                http.put('{{route('manager-permission.update',$model->id)}}', data.field).then(function (res) {
            ...
            $(".select2").val(actions || []).trigger('change');
        });
