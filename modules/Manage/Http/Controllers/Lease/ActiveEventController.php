...
use Modules\Manage\Repositories\Report\LeaseCusEventLogRepository;
...
class ActiveEventController extends Controller
{
    ...
    public function __construct(LeaseCusEventLogRepository $repository, ChartService $chartService)
    {
    ...
    public function index()
    {
        ...
        $timeType = dateType();
        return $this->view("lease.report.user.active_event", compact("provinces", "timeType"));
    }
    ...
    public function activeTrend(Request $request)
    {
        try {
            ...
            $legend = config("global.user_active_event");
            $hiddenLegend = [];
            foreach (config("global.user_active_event_display") as $url) {
