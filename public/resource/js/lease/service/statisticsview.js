...
function load_box_stock() {
    $.ajax({
        type: 'POST',
        data: {'_token': _token,'type': $("select[name='type2']").val(),'time_type': $("select[name='days2']").val()},
        dataType: 'json',
        url: adminurl + "/service/statisticsdata",
...
function load_area_data() {
    $.ajax({
        type: 'POST',
        data: {'_token': _token,datetime : $('#date2').val(),time_type: $("select[name='days2']").val()},
        dataType: 'json',
        url: adminurl + "/service/statisticsarea",
        success: function (res) {
            china_map_option = {
                tooltip: {
                    trigger: "item"
...
function load_table() {
    layui.use('table', function () {
        ...
        table.render({
            ...
            , even: true //开启隔行背景
            , size: 'sm' //小尺寸的表格
            , url: adminurl + "/service/statisticslist"//数据接口
            , where: {_token: _token, datetime: datetime}
            , page: true //开启分页
            , cols: [[ //表头
