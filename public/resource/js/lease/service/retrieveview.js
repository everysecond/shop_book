...
function load_box_stock() {
    $.ajax({
        type: 'POST',
        data: {'_token': _token,'type': $("select[name='type1']").val(),'time_type': $("select[name='days1']").val(),'datetime': $("#date1").val()},

        dataType: 'json',
        url: adminurl + "/service/retrievedata",
...
function load_area_data() {
    $.ajax({
        ...
        dataType: 'json',
        url: adminurl + "/service/retrievearea",
        success: function (res) {
            china_map_option = {
                tooltip: {
                    trigger: "item"
...
function load_table() {
    layui.use('table', function () {
        ...
        table.render({
            ...
            , even: true //开启隔行背景
            , size: 'sm' //小尺寸的表格
            , url: adminurl + "/service/retrievelist"//数据接口
