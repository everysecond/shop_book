...
function load_lease_trend() {
    $.ajax({
        ...
        dataType: 'json',
        url: adminurl + "/lease/active_trend",
...
function load_lease_time() {
    $.ajax({
        ...
        dataType: 'json',
        url: adminurl + "/lease/active_data",
        success: function (res) {
            times_option = {
                color: ['#3398DB'],
                tooltip: {
...
function load_lease_statistics() {
    layui.use('table', function () {
        ...
        table.render({
            elem: '#table_total'
            , method: 'post'
            , url: adminurl + "/lease/active_table"//数据接口
            , where: {_token: _token, agentId: agentId, dateRange: date1, type: type}
            , page: true //开启分页
            , cols: [[ //表头
                {field: 'date', title: '日期', fixed: 'left'}
                , {field: "slide/middle", title: "首页"}
                , {field: "lease-commit", title: "发起租赁"}
                , {field: "service/nearby", title: "租赁地图"}
                , {field: "store", title: "查看网点"}
                , {field: "user/wallet", title: "我的钱包"}
                , {field: "my-lease-details/", title: "查看租赁单"}
                , {field: "coupon/index", title: "优惠券"}
                , {field: "my-service-comments", title: "我的评价"}
