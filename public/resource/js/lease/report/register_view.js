...
function load_register_hour() {
    $.ajax({
        type: 'POST',
        data: {'agentId': $("select[name='register_hour_agents']").val(), dayArr: dayArr, '_token': _token},
        dataType: 'json',
        url: adminurl + "/lease/data/register_hour",
...
function load_register_total() {
    $.ajax({
        type: 'POST',
        data: {'agentId': $("select[name='register_hour_agents2']").val(), dayArr: dayArr2, '_token': _token},
        dataType: 'json',
        url: adminurl + "/lease/data/register_hour_sum",
        success: function (res) {
...
function load_day_table() {
    layui.use('table', function () {
        ...
        table.render({
            ...
            , padding: 15
            , align: "right"
            , url: adminurl + "/lease/data/table_register_hour"//数据接口
            , where: {_token: _token, agentId: agentId, dateRange: date1, type: 1}
            , page: true //开启分页
            , cols: [[ //表头
                {field: 'date', title: '日期/注册量', width: 120, fixed: 'left'}
                ...
                , {field: 'total', title: '总注册量', width: 95}
            ]]
        });
    });
}
...
function load_day_total_table() {
    layui.use('table', function () {
        ...
        table.render({
            ...
            , align: "right"
            , url: adminurl + "/lease/data/table_register_hour"//数据接口
            , where: {_token: _token, agentId: agentId, dateRange: date, type: 2}
            , page: true //开启分页
            , cols: [[ //表头
                {field: 'date', title: '日期/注册量', width: 120, fixed: 'left'}
...
function load_register_day() {
    $.ajax({
        ...
        dataType: 'json',
        url: adminurl + "/lease/data/register_day",
        success: function (res) {
            register_day_option = {
                tooltip: {
                    trigger: 'axis'
...
function load_from_data() {
    $.ajax({
        ...
        dataType: 'json',
        url: adminurl + "/lease/data/register_from",
        success: function (res) {
            from_option = {
                tooltip: {
                    trigger: 'item',
...
function load_register_total_table() {
    layui.use('table', function () {
        ...
        table.render({
            ...
            , padding: 15
            , align: "right"
            , url: adminurl + "/lease/data/register_total"//数据接口
            , where: {_token: _token, agentId: agentId, dateRange: date, type: 2}
            , page: true //开启分页
            , cols: [[ //表头
                {field: 'date', title: '日期', fixed: 'left'},
                {field: 'day_num', title: '当日注册数'},
                {field: 'total_num', title: '累计注册数'}
            ]]
        });
    });
}
