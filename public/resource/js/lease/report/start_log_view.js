...
function load_register_hour() {
    $.ajax({
        ...
        dataType: 'json',
        url: adminurl + "/lease/data/start_hour",
...
function load_day_table() {
    layui.use('table', function () {
        ...
        table.render({
            ...
            , padding: 15
            , align: "right"
            , url: adminurl + "/lease/data/table_start_hour"//数据接口
...
function load_register_day() {
    $.ajax({
        ...
        dataType: 'json',
        url: adminurl + "/lease/data/start_day",
        success: function (res) {
            register_day_option = {
                tooltip: {
                    trigger: 'axis'
...
function load_from_data() {
    $.ajax({
        ...
        dataType: 'json',
        url: adminurl + "/lease/data/start_from",
        success: function (res) {
            from_option = {
                tooltip: {
                    trigger: 'item',
...
function load_register_total_table() {
    layui.use('table', function () {
        ...
        table.render({
            ...
            , method: 'post'
            , padding: 15
            , url: adminurl + "/lease/data/start_total"//数据接口
            , where: {_token: _token, agentId: agentId, dateRange: date, type: 2}
            , page: true //开启分页
            , cols: [[ //表头
                {field: 'date', title: '日期'},
