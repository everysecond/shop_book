...
class GenerateRegisterTodayLog implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            $maxDate = LeaseRegisterLog::where("type",LeaseRegisterLog::LOG_TYPE_ONE)->max("date");
            ...
            if (!$maxDate) {
                ...
                $begin = $maxDate;
                LeaseRegisterLog::whereBetween("date", [$begin, $end])->where("type",LeaseRegisterLog::LOG_TYPE_ONE)->delete();
            }
            //24小时数组
            ...
            foreach ($dateArr as $date) {
                ...
                foreach ($nationData as $item) {
                    if ($item["register_at"] == $date) {
                        $totalNum += $item["num"];
                        $nRegisterNumArr[$item["register_hour"]] = $item["num"];
                    }
                }
                ...
                foreach ($allProvinces as $province_id => $province_name) {
                    ...
                    foreach ($provinceData as $datum) {
                        if ($datum["register_at"] == $date && $datum["province_id"] == $province_id) {
                            $totalNum += $datum["num"];
                            $pRegisterNumArr[$datum["register_hour"]] = $datum["num"];
                        }
                    }
                    $loginfo["total"] = $totalNum;
