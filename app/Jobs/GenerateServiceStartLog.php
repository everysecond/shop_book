...
class GenerateServiceStartLog implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            $maxDate = LeaseStartLog::query()
                ->whereIn("type",[LeaseStartLog::LOG_TYPE_THREE,LeaseStartLog::LOG_TYPE_FOUR])
                ->max("date");
            ...
            foreach ($dateArr as $date) {
                ...
                $loginfo["type"] = LeaseStartLog::LOG_TYPE_THREE;
                $loginfo["start_num_str"] = implode(",", $todayStartNumArr);
                $startLogs[] = $loginfo;

                $totalLogInfo["type"] = LeaseStartLog::LOG_TYPE_FOUR;
