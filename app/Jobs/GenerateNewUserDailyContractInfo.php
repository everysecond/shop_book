...
class GenerateNewUserDailyContractInfo implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            $maxDate = LeaseContractDateInfo::where("type",LeaseContractDateInfo::TYPE_ONE)->max("date");
            ...
            if (!$maxDate) {
                ...
                $begin = $maxDate;
                LeaseContractDateInfo::whereBetween("date", [$begin, $end])->where("type",LeaseContractDateInfo::TYPE_ONE)->delete();
            }
            ...
            LeaseContractDateInfo::insert($dateInfos);
            systemLog("生成新用户每日租赁合约统计信息成功");
        } catch (\Exception $e) {
            \Log::error("生成新用户每日租赁合约统计信息失败: {$e->getMessage()}");
            systemLog("生成新用户每日租赁合约统计信息失败: 详情请见laravel-log");
        }
    }
    ...
    {
        \Log::error("生成新用户每日租赁合约统计信息失败：{$exception->getMessage()}");
        systemLog("生成新用户每日租赁合约统计信息失败: 详情请见laravel-log");
    }
}
